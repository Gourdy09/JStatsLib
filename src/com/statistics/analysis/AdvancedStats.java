...
public class AdvancedStats {
    ...
    /**
     * Calculates the t-score (t-statistic) for the dataset against a known population mean.
    ...
    /**
     * Computes the z-score (standard score) for a given value within the dataset distribution.
    ...
    public double invNorm(double p, double mean, double stddev) {
        ...
        if (Math.abs(q) <= 0.425) {
            // Central region (|p - 0.5| ≤ 0.425) - higher accuracy needed here
            double r2 = q * q;
            ...
            // Tail regions - different approximation needed
            if (q <= 0) {
                r = p;
            } else {
                r = 1 - p;
            ...
            if (r <= 5.0) {
                ...
                // Extreme tail region (very small or large probabilities)
    ...
    public double[] zInterval(double cLevel, double mean, double stddev) {
        ...
        // For common confidence levels, use pre-computed z-values to avoid expensive calculation
        double z;
        if (Math.abs(cLevel - 0.90) < 1e-10) {
            ...
            // Calculate z-score for any other confidence level
